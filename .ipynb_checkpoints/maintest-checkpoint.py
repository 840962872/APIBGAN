...
for modeli in range(500):
    a = torch.load("model//generator_{}th.pth".format(modeli+50)).cuda()
    ...
    print("这是第{}次的训练，最终得分为：".format(modeli+50))
    print(1 / (1 + sqrt(score.sum(axis=0) / score.count())))

    test_data.to_csv("test_result//第{}th_score{}.csv".format(modeli+50, 1 / (1 + sqrt(score.sum(axis=0) / score.count()))),
